...
function isAlreadyCompiled(obj: unknown): obj is RunnableGraph {
  return obj != null && typeof (obj as any).stream === "function";
}

function isStateGraphBuilder(obj: unknown): obj is { compile(): RunnableGraph } {
  return obj != null && typeof (obj as any).compile === "function" && typeof (obj as any).stream !== "function";
}
 ...
 * a string). */
function extractParamListSource(fnSource: string): string | null {
  ...
  for (let i = parenIdx; i < trimmed.length; i++) {
    if (trimmed[i] === "(") depth++;
      ...
      depth--;
      if (depth === 0) return trimmed.slice(parenIdx + 1, i);
    }
  }
  return null;
}
 ...
 */
export function extractParamNames(fn: (...args: any[]) => unknown): string[] {
...
async function resolveOnce(result: unknown): Promise<RunnableGraph> {
  let resolved = result;
  if (resolved != null && typeof (resolved as any).then === "function") {
    resolved = await resolved;
  }
  if (isStateGraphBuilder(resolved)) {
    resolved = resolved.compile();
  }
  return resolved as RunnableGraph;
}
...
export async function classifyGraphExport(exportValue: unknown): Promise<GraphClassification> {
...
export class FactoryGraph {
  constructor(
    private readonly func: (...args: any[]) => unknown,
  ...
  build(
    ...
    deps: { checkpointerManager: CheckpointerManager | null; store: RunkiteStore | null },
  ): FactoryGraphBuild {
...
export class FactoryGraphBuild {
  private disposable: { dispose(): Promise<void> } | null = null;

  constructor(
    private readonly func: (...args: any[]) => unknown,
    private readonly paramNames: string[],
    private readonly config: Record<string, unknown>,
    private readonly runContext: RunFactoryContext,
    private readonly checkpointerManager: CheckpointerManager | null,
    private readonly store: RunkiteStore | null,
  ) {}
