...
function foldMessageUsage(totals, msg) {
 ...
 */
export function accumulateUsage(totals, data, skipIds, skipPrefix = 0) {
    if (data && typeof data === "object" && !Array.isArray(data) && "messages" in data) {
        ...
        if (Array.isArray(msgs)) {
            for (let i = 0; i < msgs.length; i++) {
                ...
                if (skipIds && mid && skipIds.has(mid))
                    continue;
 ...
 * usage_or_unmetered.
 */
export function usageOrUnmetered(usage, producedOutput) {
    if (usage != null)
        return usage;
    if (!producedOutput)
        return null;
