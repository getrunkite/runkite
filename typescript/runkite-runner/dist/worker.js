...
async function pollLoop(client, adapter, runnerKind, metadata, pendingCancels) {
    while (true) {
        try {
            ...
            await new Promise((resolve, reject) => {
                client.reportStatus({ runId, status, errorMessage: status === "error" ? "see error event" : "" }, metadata, (err) => {
                    if (err)
                        reject(err);
                    else
                        resolve();
