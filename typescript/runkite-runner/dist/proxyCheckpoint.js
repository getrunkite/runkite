...
function isRunNotInflight(status, body) {
    return status === 403 && body.includes("run_not_inflight");
}
function blobKey(checkpointNs, checkpointId) {
    if (!checkpointNs)
        return checkpointId;
...
function asTyped(value) {
    if (Array.isArray(value) && value.length === 2 && typeof value[0] === "string") {
        ...
        if (typeof data === "string")
            return [value[0], new TextEncoder().encode(data)];
    }
...
export class ProxyCheckpointSaver extends BaseCheckpointSaver {
    ...
    async encode(payload) {
        ...
        if (tb.length > 255)
            throw new Error("serde type name too long for proxy envelope");
    ...
    async decode(raw) {
        if (!raw.length)
            throw new Error("empty checkpoint blob");
        const n = raw[0];
        const typ = new TextDecoder().decode(raw.subarray(1, 1 + n));
        return (await this.serde.loadsTyped(typ, raw.subarray(1 + n)));
    }
    async putCas(threadId, key, body, headers, opts) {
        const putHeaders = {
            ...headers,
        ...
        if (!put.ok) {
            ...
            if (isRunNotInflight(put.status, body))
                throw new RunNotInflight();
            throw new Error(`PUT checkpoint failed: ${put.status} ${body}`);
        }
    }
    async tupleFromBlob(config, key, blob) {
        const [ns, cid] = parseBlobKey(key);
        const cfgThread = config.configurable.thread_id;
        const parentId = blob.parent_checkpoint_id;
        ...
        return {
            ...
            checkpoint: (await this.serde.loadsTyped(ct, cd)),
            metadata: (await this.serde.loadsTyped(mt, md)),
            parentConfig,
            pendingWrites,
        };
    }
    async getTuple(config) {
        ...
        if (checkpointId) {
            ...
            const init = { headers, dispatcher: this.dispatcher };
        ...
        const latestInit = { headers, dispatcher: this.dispatcher };
        const resp = await fetch(`${this.latestUrl(threadId)}?ns=${encodeURIComponent(ns)}`, latestInit);
        if (resp.status === 404)
            return undefined;
        if (!resp.ok)
            throw new Error(`GET latest checkpoint failed: ${resp.status} ${await resp.text()}`);
    ...
    async getLatestViaList(config, threadId, ns, headers) {
        const init = { headers, dispatcher: this.dispatcher };
        const resp = await fetch(`${this.url(threadId)}?limit=${LIST_FETCH_LIMIT}`, init);
        if (!resp.ok)
            throw new Error(`LIST checkpoints failed: ${resp.status} ${await resp.text()}`);
        ...
        for (const item of items || []) {
            ...
            if (itemNs !== ns)
                continue;
            const getInit = { headers, dispatcher: this.dispatcher };
            const get = await fetch(this.url(threadId, key), getInit);
            if (get.status === 404)
                continue;
            if (!get.ok)
                throw new Error(`GET checkpoint failed: ${get.status} ${await get.text()}`);
    ...
    async *list(config, options) {
        if (options?.filter) {
            throw new Error("ProxyCheckpointSaver.list(filter=...) is not supported; omit filter or use POSTGRES_DSN direct mode");
        }
        const cfgThread = config.configurable.thread_id;
        const threadId = storageThreadId(cfgThread);
        const ns = config.configurable?.checkpoint_ns ?? "";
        ...
        await this.locks.run(`${threadId}\0${key}`, async () => {
            for (let attempt = 0; attempt < CAS_MAX_ATTEMPTS; attempt++) {
                ...
                if (prev.status === 200) {
                    ...
                    if (isRunNotInflight(prev.status, body))
                        throw new RunNotInflight();
        ...
        await this.locks.run(`${threadId}\0${key}`, async () => {
            for (let attempt = 0; attempt < CAS_MAX_ATTEMPTS; attempt++) {
                ...
                if (resp.status === 404) {
                    ...
                    if (isRunNotInflight(resp.status, body))
                        throw new RunNotInflight();
                    throw new Error(`GET checkpoint failed: ${resp.status} ${body}`);
                }
                const encoded = [...(blob.writes || [])];
                ...
                try {
                    ...
                    if (err instanceof CASConflict) {
                        if (attempt === CAS_MAX_ATTEMPTS - 1) {
                            throw new Error(`checkpoint writes CAS failed after ${CAS_MAX_ATTEMPTS} attempts for ${threadId}/${key}`);
                        }
                        continue;
                    }
                    throw err;
    ...
    async deleteThread(threadId) {
        ...
        for (;;) {
            ...
            if (!items || items.length === 0)
                return;
            for (const item of items) {
                const delInit = { method: "DELETE", headers, dispatcher: this.dispatcher };
            ...
            if (items.length < LIST_FETCH_LIMIT)
                return;
        }
    }
}
