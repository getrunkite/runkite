...
**Usage / FinOps:** optional `finops` in `langgraph.json` — `pricebook`, `budgets`, `alerts.soft_pct` (default 80), `reservation` (`usd_per_run` / `tokens_per_run` / optional `hold_ttl`), and `routing` (`enabled`, `soft_pct`, `aliases`). Terminal runs with `Output.usage` write SQL `usage_events`; open creates may take `usage_holds` counted toward day caps until terminal release. Soft/hard/approach trips emit webhook `budget_alert` and audit rows (`budget_soft` / `budget_exceeded` / `budget_alert`); near-cap alias rewrites audit `budget_route` (does not bypass hard deny). Optional `on_hard_breach: cancel_inflight` drains pending/running runs in scope on hard breach (audit `budget_kill`). LangGraph (Python + TypeScript) and adapters that expose metrics (CrewAI `usage_metrics`, LlamaIndex/LangChain/AutoGen best-effort) emit top-level `usage`. Admin: `/admin/spend` plus `GET /admin-api/usage/{summary,alerts,holds,export}`. Mongo → `501` / budgets fail-open.
