...
Runkite is a control plane. Security that fits is **governance of the plane**: who can run which agent, with which connectors/tools/secrets, with kill-switches, break-glass, budgets, and an audit trail — self-hosted. (Spend dashboards / chargeback UI are out of scope; F0 caps + usage_events foundation are in.)
...
**Usage / FinOps (F0 + F1 + Admin Spend):** optional `finops` in `langgraph.json` — `pricebook` (model → `input_per_1k` / `output_per_1k` USD) and `budgets`. Terminal runs with `Output.usage` (`prompt_tokens` / `completion_tokens` / optional `cost_usd` / `model`) write SQL `usage_events` (`source=terminal_output`, upsert on `(run_id, source)`). The Python LangGraph runner sums `AIMessage.usage_metadata` into that top-level `usage` on the final `values` event. `cost_usd` wins for `usd_estimate`; else pricebook when model matches; else 0. Admin: `GET /admin-api/usage/summary` and UI `/admin/spend`. Mongo → `501`. Chargeback / alerts / price-aware routing (F3–F4) are not shipped yet.
