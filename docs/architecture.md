...
- **Proxy mode** (`POSTGRES_DSN` unset, `RUNKITE_HTTP_URL` set): LangGraph uses `ProxyCheckpointSaver`, which stores **opaque blobs** on the control plane via `/internal/checkpoints/*`. Works against **any** CP state backend (SQLite / MySQL / Mongo / Postgres). Survives runner restart without giving the runner DB credentials.
...
**HITL / mid-superstep fidelity (proxy):** full LangGraph interrupt→resume and pending writes, not a half-promise. `ProxyCheckpointSaver` packs `checkpoint` + `metadata` + `writes` (pending channel writes from `aput_writes`) into each opaque blob; `aput` preserves existing `writes`; concurrent writers merge via If-Match / ETag CAS (no silent drop). `aput_writes` without `checkpoint_id` raises. Pregel calls `aput_writes` before `aput` for a new id — both paths treat a missing blob as create-only (`If-None-Match: *`): `aput_writes` inserts a writes shell, `aput` inserts the full checkpoint; if a peer already won, `412` → GET+merge so an unconditional `aput` cannot wipe a shell. Unsupported paths fail loud (`alist(filter=...)` → `NotImplementedError`). Proven by `python/tests/test_proxy_checkpoint.py` and matrix `hitl_restart` (interrupt → kill runner → resume) on SQLite and MySQL CP with blank runner `POSTGRES_DSN`.
