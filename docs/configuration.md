...
`finops` is opt-in and control-plane-wide (first-file). Optional `pricebook` maps model id → `{input_per_1k, output_per_1k}` USD for `usage_events.usd_estimate` when Output has tokens but no `cost_usd`. Optional `budgets.tenants` / `budgets.agents` (agent keys `tenant_id/agent_id`) set UTC-day caps (`max_usd_per_day`, `max_tokens_per_day`, `max_runs_per_day`); `soft: true` allows + audits `budget_soft`, otherwise hard deny `budget_exceeded` at admission (after kill/authz/policy; break-glass does not bypass hard budgets). Requires a SQL state backend for durable usage; Mongo skips writes and Admin `/admin-api/usage/summary` returns `501`.
