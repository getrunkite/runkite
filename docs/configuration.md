...
| `RUNKITE_POSTGRES_RLS` | (unset) | When `true`/`1`/`on` with `POSTGRES_DSN`, enable FORCE row-level security on tenant-scoped tables and stamp `app.tenant_id` / `app.is_system` on each pool checkout. Defense-in-depth under application `WHERE tenant_id` filters — see [Multi-tenant](../site/support/multi-tenant.html) and [ops runbook](ops-runbook.md#4-secrets-patterns). Off by default. |
