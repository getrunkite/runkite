...
| `RUNKITE_POSTGRES_RLS` | (unset) | When `true`/`1`/`on` with `POSTGRES_DSN`, enable FORCE row-level security on tenant-scoped tables, stamp `app.tenant_id` / `app.is_system` on each pool checkout, and `SET ROLE runkite_app` (NOBYPASSRLS) on tenant requests so RLS still applies when the DSN user is a superuser (typical CI/dev). Defense-in-depth under application `WHERE tenant_id` filters — see [Multi-tenant](../site/support/multi-tenant.html). Off by default. |
