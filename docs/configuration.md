...
```json
{
  ...
  "admission_limits": {
    "per_tenant": { "max_concurrent": 50, "max_daily": 1000 },
    "per_agent": { "max_concurrent": 10, "max_daily": 200 }
  },
  "webhooks": [
  ...
  "policy": {
    ...
    "cache_ttl_ms": 0,
    "mandatory_hitl": [
      { "tenant_id": "acme", "connector": "github", "tools": ["delete_repo"] }
    ],
...
`policy` is opt-in and control-plane-wide (first-file). When the key is absent, connector access stays V1-open after runner auth + run-binding. When the `policy` section is present (even as `"policy": {}`), connector access is fail-closed and grants may be supplied via config, Admin `/admin-api/policy-grants` overlays, and/or a sync `webhook` — a grant must match `(tenant_id, agent_id, connector)` from the in-flight run (see [Trust & governance](trust-governance.md)). Optional `webhook` is a sync PolicyProvider (same HMAC / fail-closed culture as `preflight_hooks`, not the async `webhooks` path); its response may use `"effect": "pending"` for connector tool HITL (Admin `/admin-api/pending-actions` on SQL backends — approve is Admin-only and bound to the pending row's `args_digest`; the agent must retry the same arguments after approve; a different amount is a new row). Optional `predicates` are config-only argument checks on connector MCP `tools/call` (not in-graph `AuthorizeTool`): after a matching grant, before the sync webhook. `effect` is `deny` or `pending` only — predicates never grant access. `when.path` is a dotted map path (numeric segments index arrays); missing path does not match. Ops: `eq` `neq` `gt` `gte` `lt` `lte` `in` `contains` `exists`. Numeric compares use JSON numbers only (string `"100"` vs number `100` does not match). Empty `agent_id` = whole tenant; empty `tool` = every tool on that connector. Invalid rows are skipped at engine start (logged), not silently treated as allow. Optional `cache_ttl_ms` (default `0` = off) caches allow/deny decisions for that duration in a 10,000-entry LRU; the key is `(stage, tenant_id, agent_id, principal, connector, tool, args_digest)` so a BYO PDP that differs by principal or a predicate that differs by amount cannot leak Alice's allow to Bob (or a $50 allow to a $250 call). Empty digest (session / `run.create`) keeps the previous name-only behavior. Any webhook field that changes the decision must stay in that key (or leave cache off). Pending is never cached. Optional `mandatory_hitl` is a list of `{tenant_id, agent_id?, connector, tools?}` deployment defaults that force matching `tools/call` **allows** to pending (`reason_code: mandatory_hitl`) even when grants/webhook would allow — hard deny is not elevated; empty `agent_id` = whole tenant; empty `tools` = all tools on that connector. Durable SQL overlays (`/admin-api/mandatory-hitl`) win on the same `(tenant_id, agent_id, connector)` without redeploy. Optional `siem` is an **async** export sink for `policy_decision` events only (same delivery / HMAC / dead-letter path as top-level `webhooks`; never blocks Decide). Decisions are written to SQL `audit_events` when `audit` is true (default); Mongo skips durable audit. Every Decide also emits an OTel span event `policy.decide` on the current request span when `OTEL_EXPORTER_OTLP_*` is set. When `run_events` is true (default), connector policy denials / pending also publish a `tool_auth` RunEvent onto the run's stream (see [Trust & governance](trust-governance.md)); set `"run_events": false` to disable. Durable grant overlays (Admin CRUD on SQL) are separate from this config block — see `/admin-api/policy-grants`. There is no Admin CRUD for predicates this release.
...
`admission_limits` is opt-in and control-plane-wide (first-file), separate from `rate_limit` on purpose: these are **occupancy/quota** ceilings counted from the state store, not request-rate token buckets. Any subset of `per_tenant` / `per_agent` with `max_concurrent` and/or `max_daily` may be set (`<= 0` or omit = unlimited). `max_concurrent` counts runs in `pending` or `running`; `max_daily` counts runs with `created_at` on or after the start of the current **UTC** day. Enforced in the shared run-creation path via `CreateRunAdmitted`: a per-tenant / per-agent scope lock (Postgres advisory xact lock, MySQL `GET_LOCK`, SQLite `BEGIN IMMEDIATE`, Mongo lock-doc + transaction) then COUNT then INSERT on the **same** connection/transaction, so a burst of concurrent creates cannot all pass a stale COUNT. Exceeding a limit returns `429` with `Retry-After` (concurrent: `1`; daily: seconds until next UTC midnight). A rejected create may still leave an idle auto-created thread (claim is released); that is preferred over a non-atomic early check.

`finops` is opt-in and control-plane-wide (first-file). Optional `pricebook` maps model id → `{input_per_1k, output_per_1k}` USD for `usage_events.usd_estimate` when Output has tokens but no `cost_usd` (Admin Spend labels this **Est. USD** — estimate, not invoice). When a gateway/runner reports inline `cost_usd` (OpenRouter `usage.cost` / `total_cost`), that figure wins over the pricebook. With a non-empty pricebook, a terminal run that has tokens, no `cost_usd`, and a model id missing from the book audits `usage_unpriced` (empty pricebook = tokens-only, no alert; a present `$0`-rate row is intentional free tier). A terminal run whose reply the runner recognizes as an AI turn but from which it could extract zero token/cost data in any shape at all (an unrecognized provider/framework `usage_metadata` shape) audits `usage_unmetered` instead — this fires regardless of whether a pricebook is configured, since the gap here is missing token data, not missing pricing. Optional `budgets.tenants` / `budgets.agents` (agent keys `tenant_id/agent_id`) set UTC-day caps (`max_usd_per_day`, `max_tokens_per_day`, `max_runs_per_day`); `soft: true` allows + audits `budget_soft`, otherwise hard deny `budget_exceeded` at admission (after kill/authz/policy; break-glass does not bypass hard budgets). Optional `alerts.soft_pct` (default 80) emits approach `budget_alert` audits/webhooks under a hard cap. Approach alerts are deduped once per tenant/agent/scope/kind/UTC day per process (soft/hard still emit every time; multi-replica may still duplicate). Optional `reservation.usd_per_run` / `tokens_per_run` places optimistic `usage_holds` counted toward day caps until terminal release (0 = off). Optional `reservation.agents` map (`tenant_id/agent_id` → `{usd_per_run, tokens_per_run}`) overrides the global hold size per agent so expensive and cheap agents need not share one estimate. Optional `reservation.hold_ttl` (Go duration, e.g. `"24h"`) expires open holds older than that age so a crashed runner cannot pin day caps forever (omit / `"0"` = no sweeper; keep TTL longer than your longest expected run or an in-flight hold can expire and under-reserve until terminal). Optional `routing.enabled` + `routing.aliases` rewrites to a cheaper configured agent near soft_pct (audits `budget_route`; never bypasses hard deny). Subscribe webhooks to `budget_alert` for outbound delivery. Optional `on_hard_breach: "cancel_inflight"` drains pending/running runs in the breached tenant/agent scope when a hard cap is already exceeded (default: deny new creates only). Requires a SQL state backend for durable usage; Mongo skips writes and Admin `/admin-api/usage/*` returns `501`.

**Live Admin overlays (SQL):** the file `finops` section is the bootstrap baseline. Admins can change pricebook / budgets (and the rest of the FinOps patch shape) at runtime via `GET|PUT|DELETE /admin-api/finops` and the Spend page **Live FinOps config** editor — same pattern as policy grants (writer hot-reload + ≤15s sibling poll). Overlay map keys win over the file; clearing the overlay restores file-only effective config. Writes are validated server-side (bounds, agent key shape, swapped-rate heuristics). No control-plane restart required for overlay changes.
...
| `RUNKITE_POSTGRES_RLS` | (unset) | When `true`/`1`/`on` with `POSTGRES_DSN`, enable FORCE row-level security on tenant-scoped tables, stamp `app.tenant_id` / `app.is_system` on each pool checkout, and `SET ROLE runkite_app` (NOBYPASSRLS) on tenant requests so RLS still applies when the DSN user is a superuser (typical CI/dev). The DSN role needs `CREATEROLE` (or superuser) so Init can `CREATE ROLE runkite_app` and `GRANT … WITH SET TRUE` (Postgres 16+). After Init, `SET ROLE` failures are fail-closed (no silent bypass). Turning the flag off and restarting drops Runkite's policies / FORCE so sticky deny-all does not linger. Defense-in-depth under application `WHERE tenant_id` filters — see [Multi-tenant](../site/support/multi-tenant.html). Off by default. |
| `VAULT_ADDR` | (unset) | HashiCorp Vault base URL for connector `auth.secret_ref` `vault:` scheme (KV v2 `…/data/…` or KV v1 without `/data/`). See [Secrets](../site/support/secrets.html) / [ops runbook](ops-runbook.md#4-secrets-patterns). |
...
| `VAULT_ALLOWED_PREFIX` | `secret/data/runkite/` | Fail-closed allowlist for `vault:` paths after `path.Clean` (rejects `..`). Default matches KV v2; for KV v1 set e.g. `secret/runkite/`. |
