...
- **Direct-mode LangGraph checkpoints are still not tenant-scoped.** `store_items` / `vector_items` follow `RunAssignment.tenant_id` (and runner proxy calls send `X-Runkite-Tenant-Id` on `/internal/*`). LangGraph's own Postgres checkpointer tables (`checkpoints`, `checkpoint_blobs`, `checkpoint_writes`) have no `tenant_id` column -- isolation there is only by `thread_id`. For checkpoint-level multi-tenant isolation, unset `POSTGRES_DSN` on runners and use `RUNKITE_HTTP_URL` (proxy mode).
- **`X-Runkite-Tenant-Id` on `/internal/*` is self-reported by the runner.** After runner-token auth passes, the control plane honors that header for proxy-mode store/vector scope without cross-checking it against the assigned run's `tenant_id` on record. Runner tokens are per-kind, not per-tenant or per-run, so a compromised runner that already holds a kind token can target any named tenant's `store_items`/`vector_items` namespace -- not only the run it was assigned. Same trust class as connector credential vending (runners already receive secrets for whatever job they dequeue). Deployments that need stronger isolation should run separate runner-kind token pools per trust boundary, or keep store access in direct mode where the runner never asks the control plane to re-scope. Hardening (require a run id and reject a mismatched claimed tenant) is open.
