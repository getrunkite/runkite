...
**Authorization**: `permissions` set on an API key, JWT claim, or webhook response are enforced -- `read` is required for GET/HEAD, `write` for everything else (`admin` bypasses both). Optional `agents:<agent_id>:run` is **not** blanket write: it satisfies route-level `write` only on run-create routes (`POST /runs`, `POST /runs/stream`, `POST /runs/wait`, and the matching `POST /threads/{id}/runs[...]` variants). It does **not** authorize DELETE/cancel/store/thread mutations. Inside `createRunCtx`, `CanRunAgent` still requires that exact agent (blanket `write`/`admin` allow any agent). Pair with `read` if the client needs to stream/wait after create, e.g. `["read", "agents:echo:run"]`. When `auth.type` is `api_key`, `jwt`, or `webhook`, **empty permissions fail closed by default** (`403`) -- every caller must carry an explicit allow-list. Set `"strict_permissions": false` only for local/dev if you intentionally want the old empty=unrestricted behavior. `"strict_permissions": true` is always fail-closed (same as the default for those provider types).
...
**Admin UI login** (`POST /admin-api/session`) exchanges a pasted API key/JWT for an httpOnly `runkite_admin_session` cookie; the browser never keeps the credential in JavaScript. Mutating dashboard calls send `X-CSRF-Token` from the login response. `GET /admin-api/session` rehydrates CSRF after refresh; `DELETE /admin-api/session` logs out. Curl/OpenAPI keep using `Authorization: Bearer` on `/admin-api/*` with no CSRF. When `REDIS_URL` is set, session records are stored in Redis (shared across control-plane replicas, same auto pattern as Redis rate limits); otherwise they stay in process memory.

**Runner auth** is separate: in local mode runners are trusted implicitly. In production, set `RUNNER_TOKEN_<kind>` env vars — one token or a **comma-separated allowlist** of equally valid tokens per runner type, validated on every gRPC call and `/internal/*` HTTP request. Runners still present a single `RUNNER_TOKEN`. The allowlist supports dual-token rotation and independently revocable fleets for the same kind (not unique-per-pod secrets under a Deployment). Set `RUNNER_TENANTS_<kind>` (comma-separated tenant ids, same kind encoding as tokens) to restrict which tenants that kind may use on unbound `/internal/*` routes. When client-facing auth and `RUNNER_TOKEN_*` are both configured, **`serve` fail-closes unless every tokenized kind has a `RUNNER_TENANTS_*` list** (use `default` for single-tenant). A missing tenant header is treated as `default` for the allow check. Tenant lists are per-kind, not per fleet token. Allowlist changes require a control-plane restart (tokens load at process start).
