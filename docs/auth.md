...
- **`X-Runkite-Tenant-Id` on `/internal/*` is self-reported by the runner.** After runner-token auth passes, the control plane honors that header for proxy-mode store/vector scope without cross-checking it against the assigned run's `tenant_id` on record. By default, kind tokens are not per-tenant, so a compromised runner that holds a kind token can target any named tenant's `store_items`/`vector_items` namespace -- same trust class as connector credential vending. Optional `RUNNER_TENANTS_<kind>` allow-lists shrink that blast radius to the listed tenants (missing header → `default` must be listed explicitly if you want it). Stronger isolation still means separate kind-token pools per trust boundary, direct-mode store access, or future hardening (require a run id and reject a mismatched claimed tenant).
