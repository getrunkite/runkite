...
**Crash recovery, and its real boundary**: the control plane tracks every dequeued job as in-flight, and an unacked job past a short max-age (6s) is automatically reclaimed and redelivered to a live runner -- this closes the highest-risk window (a runner dying between `GetJob` and actually starting work, the "zombie GetJob" case), combined with gRPC keepalive (detects a dead runner connection in ~4s) and a same-instant `ctx.Err()` check in `GetJob` -- verified by killing a runner mid-interrupt and resuming against a fresh process with zero added delay. **The window this protects is narrower than "mid-job" implies, though**: the job is Ack'd (removed from in-flight tracking) the moment the runner emits its *first* event, not when it completes -- confirmed via live multi-instance testing (killing a runner immediately after it started a multi-step job left the run permanently stuck, with no automatic recovery, because it had already emitted one event and was no longer being watched). A runner crash *after* that point, at any stage of a long-running job, is not yet recovered automatically -- closing that gap needs an ongoing heartbeat during execution (not just a one-time Ack), which is planned but not yet built. With the Redis transport, in-flight tracking (for the window it does cover) lives in Redis itself, not in any one control-plane process's memory -- so it's correctly shared across a multi-node control plane. Verified live with a Redis `MONITOR` trace during real multi-instance traffic, not inferred from an ambiguous kill test: the `HSET`/`ZADD` recording a job in-flight came from one control-plane replica's IP, and the `HDEL`/`ZREM` Acking it came from a *different* replica's IP -- cross-instance sharing under nginx's round-robin, confirmed on the wire. Separately verified the reclaim path itself by directly seeding a deliberately-stale in-flight entry (simulating a replica that dequeued a job and then died before its runner's first event) and confirming a live, different replica's reaper picked it up and correctly re-queued it within one reaper-ticker cycle. The in-process (zero-dependency, single-instance) transport is, as its name says, still local to that one process by design.
...
- **A runner crash mid-execution (after its first event) is not automatically recovered, in any deployment topology.** In-flight tracking currently Acks a job on the runner's *first* emitted event, not on completion -- a deliberate design for the highest-risk window (crash between dequeue and starting work), but it means a crash at any later point in a long-running job leaves the run permanently stuck, with no automatic redelivery. Confirmed via live testing, not theoretical. Closing this needs an ongoing heartbeat during execution (the runner periodically refreshing a liveness signal, not just once), so the same reclaim mechanism can catch a crash at any point -- planned, not yet built.
- **No fencing for a reclaimed job's original runner.** With the Redis transport, in-flight job tracking (Ack/Nack/reclaim), for the window it does cover, is shared across control-plane replicas (live-verified against a real multi-instance topology), so a crashed replica no longer loses or duplicates a job on its own within that window. What's still missing: if a runner has a transient network blip that makes it *look* dead (triggering a reclaim and redelivery to a second runner) but the original runner actually finishes anyway, nothing yet stops it from reporting a stale result after the second runner has already taken over. A generation/fencing token to close this is planned but not yet built, and depends on the heartbeat mechanism above existing first. The in-process (zero-dependency, single-instance) transport's in-flight tracking is local to that one process by design, not a gap -- there's only one process to track it in.
