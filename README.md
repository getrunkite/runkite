...
**`admin_keys`** (optional, under the same `auth` object) is an independent credential set accepted **only** for `/admin-api/*` -- every key implicitly grants `admin`. Useful when the primary provider is short-lived SSO and an operator wants a stable break-glass key for the dashboard without minting JWTs. It is additive: a primary credential that itself carries `admin` still works.

**Misconfiguration footgun**: `admin_keys` alone, with no primary `auth.type` (`api_key` / `jwt` / `webhook`), provides **zero** access control. Middleware tries the admin key first for `/admin-api/*`, and if that fails *and no primary provider is configured*, the request falls through to local/dev trust-everyone -- for the **entire** API, not just the dashboard. That fallthrough is intentional (preserves zero-config local ergonomics when `auth` is absent), but it means configuring only `admin_keys` looks like you locked things down when you haven't. Pair `admin_keys` with a real primary provider; unauthenticated requests then correctly get `401`.
...
**Auth**: the dashboard's login screen accepts whatever credential the configured `auth` provider expects (an API key or a JWT) and requires the `admin` permission specifically -- `read`/`write` are not enough, even for viewing the dashboard, since every `/admin-api/*` route sees across every tenant. An empty `permissions` list is still unrestricted (the same backward-compatible convention as the rest of the API). Optional `auth.admin_keys` are also accepted on `/admin-api/*` only (see Auth above). With no primary `auth` provider configured at all (local/dev mode), the dashboard skips the login screen entirely -- there's no credential to log in with, and the API is fully open. **Do not configure `admin_keys` without also setting a primary `auth.type`** -- alone they do not gate anything (same section's misconfiguration footgun).
