...
**What's still open**: a rarer edge case -- a runner has a transient blip, gets reclaimed and its job redispatched to a second runner, but the *original* runner's blip was temporary and it finishes anyway, reporting a stale result after a second runner already took over. Nothing currently stops the control plane from accepting that stale report (no fencing/generation token yet). This only matters once a job is actually being reclaimed mid-execution, which is now possible (that's the whole point of the fix above) -- tracked as the next hardening pass, not blocking anything above it.
...
- **No fencing for a reclaimed job's original runner.** With the Redis transport, in-flight job tracking (Ack/Nack/reclaim/Heartbeat) is shared across control-plane replicas and covers a job's whole execution now, not just its start (live-verified against a real multi-instance topology -- see "Crash recovery" above), so a crashed runner no longer leaves a run permanently stuck. What's still missing: if a runner has a transient network blip that makes it *look* dead (triggering a reclaim and redelivery to a second runner) but the original runner actually finishes anyway, nothing yet stops it from reporting a stale result after the second runner has already taken over. A generation/fencing token to close this is the next hardening pass, not yet built -- this only matters now that mid-execution reclaim is actually possible, which it wasn't before the heartbeat fix. The in-process (zero-dependency, single-instance) transport's in-flight tracking is local to that one process by design, not a gap -- there's only one process to track it in.
