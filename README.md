...
**Authorization**: `permissions` set on an API key, JWT claim, or webhook response are enforced -- `read` is required for GET/HEAD, `write` for everything else (`admin` bypasses both). An **empty** permissions list means unrestricted access by default (backward compatible -- authenticating without configuring fine-grained permissions keeps full access; you only get restricted by explicitly granting a limited list). Set `"strict_permissions": true` on the same `auth` object to fail closed instead: empty (or only foreign IdP scopes that are not `read`/`write`/`admin`) denies every request with `403`. Use that for production SSO/api_key deployments where every caller must carry an explicit allow-list.
...
- **Admin UI credentials still live in `localStorage`.** CSP/`X-Frame-Options`/nosniff headers reduce XSS and clickjacking blast radius for `/admin/`, but an XSS that does run can still read the stored Bearer token -- an httpOnly cookie session (with CSRF) would close that; not implemented yet.
...
- **Admin UI list pages still soft-cap at 1000 rows** (Agents/Threads/Runs/Registry) with no pagination UI yet -- overview totals themselves use real `COUNT`/`GROUP BY` aggregates and are not capped. Webhook dead-letter redeliver still scans the first 1000 DLs rather than a Get-by-ID.
