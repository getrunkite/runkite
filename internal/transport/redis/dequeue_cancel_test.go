...
// inside a single, long blocking pop for up to the FULL GetJob timeout
...
// cancelled could be atomically moved by Redis for delivery to this
// connection, then lost when the connection tore down mid-response.
// (With BLMOVE the job lands on rk:inflight:pending rather than vanishing;
// the original flake was against BRPOP.) Capping each individual BLMOVE
// call's blocking duration (dequeueBlockCap) and checking ctx.Err() at
// the top of each loop iteration means a cancelled context is noticed
// within one cap-sized tick instead of only whenever the single long
// pop happens to surface it -- this test proves that reaction-time bound
// directly, without needing to reproduce the exact concurrent-push race
// live (that was reproduced and confirmed separately against the real
...
func TestDequeue_ContextCancelDetectedWithinBlockCap_NotFullTimeout(t *testing.T) {
	...
	// bug was that Dequeue could stay blocked in one BLMOVE call for
	...
	if elapsed > 4*time.Second {
		t.Fatalf("Dequeue took %v to notice context cancellation (want well under the 10s timeout) -- "+
			"regression back to blocking for the full remaining duration on one BLMOVE call", elapsed)
	}
