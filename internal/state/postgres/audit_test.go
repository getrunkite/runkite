...
func TestAuditEvents_WriteAndMigrateV2(t *testing.T) {
	...
	// Downgrade v10→v9 (opaque version col) then v9→v8 (opaque table) then
	// v8→v7 (mandatory_hitl) then v7→v6 (break_glass) then v6→v5 (index)
	// then v5→v4→v3→v2→v1: drop kill_switches, pending_actions, policy_grants,
	...
	if !tableExists(t, ctx, s, "audit_events") || !tableExists(t, ctx, s, "policy_grants") || !tableExists(t, ctx, s, "pending_actions") || !tableExists(t, ctx, s, "kill_switches") || !tableExists(t, ctx, s, "break_glass_windows") || !tableExists(t, ctx, s, "mandatory_hitl_rules") || !tableExists(t, ctx, s, "opaque_checkpoints") {
		t.Fatal("audit_events/policy_grants/pending_actions/kill_switches/break_glass_windows/mandatory_hitl_rules/opaque_checkpoints missing after re-Init")
	}
