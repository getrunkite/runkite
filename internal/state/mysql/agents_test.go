...
func execTruncate(ctx context.Context, s *mysql.Store) (bool, error) {
	for _, tbl := range []string{"agent_schemas", "agent_versions", "agents", "thread_checkpoints", "runs", "threads", "store_items", "webhook_dead_letters", "run_cache", "cron_claims", "cron_schedules"} {
