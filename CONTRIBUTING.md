...
At minimum, a new runner needs: `GetJob`/`ReportStatus`/`StreamEvents`/`WatchCancels` gRPC calls, the `RunAssignment`/`RunEvent` JSON shapes (see [`runner-protocol/PROTOCOL.md`](runner-protocol/PROTOCOL.md), the [public protocol repo](https://github.com/getrunkite/runner-protocol), and [`docs/runners.md`](docs/runners.md)), and either direct-mode or proxy-mode checkpoint/store access (proxy mode -- calling the control plane's own HTTP API -- is the simpler starting point; see Checkpoint/Store dual mode in [`docs/architecture.md`](docs/architecture.md)). Spec issues prefer the protocol repo; Runkite implementation issues stay here.
