...
checkpoint_id and load it via GET .../latest before each run.
...
class OpaqueCheckpointClient:
    """GET latest / PUT opaque bytes for one runner process."""
    ...
    def _put_url(self, thread_id: str, checkpoint_id: str = ADAPTER_CHECKPOINT_ID) -> str:
        return (
            f"{self._base}/internal/checkpoints/"
            f"{quote(thread_id, safe='')}/{quote(checkpoint_id, safe='')}"
        )

    async def get_latest(self, thread_id: str) -> bytes | None:
        """Return latest opaque blob for thread, or None if missing."""
        if not thread_id:
            return None
        client = await self._http()
        resp = await client.get(self._latest_url(thread_id), headers=self._client_headers())
    ...
    async def put(self, thread_id: str, data: bytes, *, checkpoint_id: str = ADAPTER_CHECKPOINT_ID) -> None:
        ...
        resp = await client.put(self._put_url(thread_id, checkpoint_id), content=data, headers=headers)
