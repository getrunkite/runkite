...
test-python:
	@if [ -x python/.venv/bin/python ]; then \
		...
		PYTHONPATH=python python/.venv/bin/python python/tests/test_usage_accumulate.py && \
		...
		PYTHONPATH=python python3 python/tests/test_usage_accumulate.py && \
...
# Kind Ingress + NetworkPolicy (K3): ingress-nginx, Host: runkite.local
# via :8080, echo_agent success, runner deny-inbound probe.
kind-helm-net:
	@bash scripts/kind-helm-net.sh

# K0–K3 kind Helm suite (sequential). See docs/k8s-kind-proof.md.
# Not an EKS soak; cluster installs stay Compatible until K4.
kind-helm-all:
