...
def _build_admin_spec() -> dict:
    ...
    # cursor and offset are mutually exclusive; X-Next-Cursor resumes keyset pages.
    admin_page = (
        {"name": "limit", "in": "query", "required": False, "schema": {"type": "integer", "default": 50, "maximum": 200}},
        {"name": "offset", "in": "query", "required": False, "schema": {"type": "integer", "default": 0, "minimum": 0},
         "description": "Offset paging (mutually exclusive with cursor)."},
        {"name": "cursor", "in": "query", "required": False, "schema": {"type": "string"},
         "description": "Opaque keyset resume token from a prior X-Next-Cursor response (mutually exclusive with offset)."},
    )
    admin_list_headers = {
        "X-Next-Cursor": {
            "description": "Opaque resume token when another page exists (len(items) == limit).",
            "schema": {"type": "string"},
        }
    }
            ...
            "schemas": {
                ...
                "FinOpsBudgetCap": {
                    ...
                    "properties": {
                        ...
                        "soft": {"type": "boolean", "description": "true = allow + audit budget_soft; false = hard deny budget_exceeded."},
                    },
                    "title": "FinOpsBudgetCap",
                },
                "FinOpsConfigPatch": {
                    "type": "object",
                    "description": "Partial finops.Config JSON — same shape as the file langgraph.json finops section. Any field omitted is left to the other side of the merge (file baseline for GET's overlay/effective view, or 'no change' semantics do not apply on PUT — PUT replaces the entire stored overlay document).",
                    "properties": {
                        "pricebook": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "object",
                                "properties": {"input_per_1k": {"type": "number"}, "output_per_1k": {"type": "number"}},
                                "title": "FinOpsModelPrice",
                            },
                        },
                        "budgets": {
                            "type": "object",
                            "properties": {
                                "tenants": {"type": "object", "additionalProperties": _ref("FinOpsBudgetCap"), "description": "Keyed by tenant_id."},
                                "agents": {"type": "object", "additionalProperties": _ref("FinOpsBudgetCap"), "description": "Keyed by tenant_id/agent_id."},
                            },
                        },
                        "alerts": {"type": "object", "properties": {"soft_pct": {"type": "number", "minimum": 0, "maximum": 100}}},
                        "reservation": {
                            "type": "object",
                            "properties": {
                                "usd_per_run": {"type": "number"},
                                "tokens_per_run": {"type": "integer"},
                                "hold_ttl": {"type": "string", "description": "Go duration string, e.g. \"24h\"."},
                                "agents": {"type": "object", "additionalProperties": {"type": "object", "properties": {"usd_per_run": {"type": "number"}, "tokens_per_run": {"type": "integer"}}}, "description": "Keyed by tenant_id/agent_id."},
                            },
                        },
                        "routing": {
                            "type": "object",
                        ...
                        "on_hard_breach": {"type": "string", "enum": ["", "cancel_inflight"]},
                    },
                    "title": "FinOpsConfigPatch",
                },
                "AdminFinOpsView": {
                    "type": "object",
                    "description": "File baseline, live SQL overlay, and the merged effective config the control plane actually applies (file ∪ overlay, overlay wins on set keys).",
                    "properties": {
                        "file": _ref("FinOpsConfigPatch"),
                        "overlay": _ref("FinOpsConfigPatch"),
                        "effective": _ref("FinOpsConfigPatch"),
                    ...
                    "required": ["file", "overlay", "effective", "has_overlay"],
                    "title": "AdminFinOpsView",
                },
            },
        },
        "paths": {
            ...
            "/admin-api/threads": {
                ...
                "post": {"tags": ["Admin"], "summary": "Create Thread (admin try)", "description": "Creates a thread in ?tenant_id= (default 'default') for the Admin Try-agent playground. Same body as public POST /threads.", "operationId": "admin_create_thread", "parameters": [{"name": "tenant_id", "in": "query", "required": False, "schema": {"type": "string", "default": "default"}}], "requestBody": {"required": False, "content": {"application/json": {"schema": {"type": "object"}}}}, "responses": {**_json_response("200", "Success", _ref("AdminThreadView"))}},
            },
            ...
            "/admin-api/threads/{thread_id}/runs": {"get": {"tags": ["Admin"], "summary": "List Thread Runs (admin)", "operationId": "admin_list_thread_runs", "parameters": [t_id, *admin_page], "responses": {"200": {"description": "Success", "headers": admin_list_headers, "content": {"application/json": {"schema": _array_of(_ref("AdminRunView"))}}}, "400": {"description": "Invalid cursor or cursor+offset", "content": {"application/json": {"schema": _ref("ErrorResponse")}}}}}},
            "/admin-api/threads/{thread_id}/runs/stream": {"post": {"tags": ["Admin"], "summary": "Create And Stream Thread Run (admin try)", "description": "Same as public POST /threads/{id}/runs/stream, scoped to ?tenant_id=. Used by the Admin Try-agent playground (Admin session cookie is Path=/admin-api only).", "operationId": "admin_create_and_stream_thread_run", "parameters": [t_id, {"name": "tenant_id", "in": "query", "required": False, "schema": {"type": "string", "default": "default"}}], "requestBody": {"required": True, "content": {"application/json": {"schema": {"type": "object", "description": "Same shape as public RunCreate (agent_id, input, command, stream_mode, ...)"}}}}, "responses": {"200": {"description": "SSE stream", "content": {"text/event-stream": {"schema": {"type": "string"}}}}, "400": {"description": "Invalid body", "content": {"application/json": {"schema": _ref("ErrorResponse")}}}}}},
            "/admin-api/runs": {"get": {"tags": ["Admin"], "summary": "List All Runs", "operationId": "admin_list_runs", "parameters": [
                ...
                *admin_page,
            ], "responses": {"200": {"description": "Success", "headers": admin_list_headers, "content": {"application/json": {"schema": _array_of(_ref("AdminRunView"))}}}, "400": {"description": "Invalid cursor or cursor+offset", "content": {"application/json": {"schema": _ref("ErrorResponse")}}}}}},
            ...
            "/admin-api/audit-events": {"get": {"tags": ["Admin"], "summary": "List Audit Events", "description": "Policy decisions newest-first. SQL state backends (Postgres/MySQL/SQLite); Mongo returns 501.", "operationId": "admin_list_audit_events", "parameters": [
                ...
                {"name": "since", "in": "query", "required": False, "schema": {"type": "string", "format": "date-time"}, "description": "Inclusive lower bound on ts (RFC3339)."},
                {"name": "until", "in": "query", "required": False, "schema": {"type": "string", "format": "date-time"}, "description": "Exclusive upper bound on ts (RFC3339)."},
                *admin_page,
            ], "responses": {
            ...
            "/admin-api/finops": {
                "get": {"tags": ["Admin"], "summary": "Get Live FinOps Config", "description": "File baseline, live SQL overlay, and merged effective finops config (pricebook/budgets/alerts/reservation/routing). SQL state backends only.", "operationId": "admin_get_finops", "responses": {
                    "200": {"description": "Success", "content": {"application/json": {"schema": _ref("AdminFinOpsView")}}},
                    "501": {"description": "State backend is non-SQL (finops overlay requires Postgres/MySQL/SQLite)", "content": {"application/json": {"schema": _ref("ErrorResponse")}}},
                }},
                "put": {"tags": ["Admin"], "summary": "Replace Live FinOps Overlay", "description": "Replaces the entire stored overlay document with the given partial finops.Config (full replace, not a deep merge onto the previous overlay — omit a key to remove it from the overlay). Validated server-side (rate/budget bounds, agent key shape tenant_id/agent_id, swapped-rate heuristics). Reloads the effective config on this replica immediately; siblings converge within ~15s via poll.", "operationId": "admin_put_finops", "requestBody": {"required": True, "content": {"application/json": {"schema": _ref("FinOpsConfigPatch")}}}, "responses": {
                    "200": {"description": "Success — returns the updated view (same shape as GET)", "content": {"application/json": {"schema": _ref("AdminFinOpsView")}}},
                    "400": {"description": "Invalid JSON or a value failed validation (out-of-range rate/budget, malformed agent key, swapped input/output rate heuristic, etc.)", "content": {"application/json": {"schema": _ref("ErrorResponse")}}},
                    "501": {"description": "State backend is non-SQL (finops overlay requires Postgres/MySQL/SQLite)", "content": {"application/json": {"schema": _ref("ErrorResponse")}}},
                }},
                "delete": {"tags": ["Admin"], "summary": "Clear Live FinOps Overlay", "description": "Deletes the stored overlay; effective config reverts to the file langgraph.json baseline on this replica immediately, siblings within ~15s.", "operationId": "admin_delete_finops", "responses": {
